...
class FWI_LBFGS(nn.Module):
    ...
    def forward(self, model, gradt=False, scipy=False):
        ...
        for it in range(self.num_batches):
            prop = deepwave.scalar.Propagator({'vp': model}, self.dx,
                                              pml_width=[0, 20, 20, 20, 0, 0])
            batch_src_wvl = self.source_wavelet[:, it::self.num_batches].to(self.device)
            batch_data_true = self.data_true[:, it::self.num_batches].to(self.device)
            batch_x_s = self.x_s[it::self.num_batches].to(self.device)
            batch_x_r = self.x_r[it::self.num_batches].to(self.device)
            data_pred = prop(batch_src_wvl, batch_x_s, batch_x_r, self.dt)
            self.nWE += 1
            loss = self.criterion(data_pred, batch_data_true)
    ...
    def callback(self, x, x_true, velocity_error, data_residual, nWEs):
        ...
        return np.array(velocity_error), np.array(data_residual), np.array(nWEs)
