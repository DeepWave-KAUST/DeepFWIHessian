...
class FWI_LBFGS(nn.Module):
    def __init__(self, data_true: torch.Tensor, source_wavelet: torch.Tensor, x_s: torch.Tensor, 
                 ...
                 model_dims: Tuple[int, int], msk: torch.Tensor, scaling: float, mask: Optional[torch.Tensor] = None, 
        ...
        self.device = device
        self.sn = 499 # amount of padding to the data
        self.mask = mask_d
    ...
    def forward(self, model, gradt=False, scipy=False):
        ...
        for it in range(self.num_batches):
            batch_data_pred = deepwave.scalar(model,
                                   self.dx,
                                   self.dt,
            ...
            batch_data_pred *= self.mask[it::self.num_batches]
            batch_data_pred2 = torch.nn.functional.pad(batch_data_pred[:, :, self.sn:], (0, self.sn, 0, 0, 0, 0), 'constant', 0)
            # Get the corresponding observed data batch
            batch_observed_data = self.data_true[it::self.num_batches].to(self.device)

            # Compute loss between predicted and observed data, ignoring the first `ignore_n_samples`
            loss = self.criterion(
                batch_data_pred2.squeeze(), 
        ...
        if gradt:
            # Create a taper for the shallower and deeper parts of the gradients
            taper = cosine_taper(nz=self.model_dims[0], nx=self.model_dims[1], top_width=100, bottom_width=150)
            taper = torch.tensor(taper).float().to(self.device)
            # Scale gradient
            running_loss = running_loss / self.scaling
            # Clip the gradient
            torch.nn.utils.clip_grad_value_(model, torch.quantile(model.grad.detach().abs(), 0.98))
            # Smooth the gradient
            model.grad[:] = apply_1d_gaussian_filter(model.grad.detach(), 8, 20)
            # Mask the top part of the gradient and taper it
            model.grad = (model.grad * self.msk * taper) / self.scaling 
