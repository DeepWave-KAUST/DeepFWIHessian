...
def scalar(
    ...
    (wfc, wfp, psiy, psix, zetay, zetax, receiver_amplitudes, dwdv) = \
        scalar_func(
    ...
    return wfc, wfp, psiy, psix, zetay, zetax, receiver_amplitudes, dwdv
...
class ScalarForwardFunc(torch.autograd.Function):
    ...
    def forward(ctx, v, source_amplitudes, wfc, wfp, psiy, psix, zetay, zetax,
        ...
        if (ctx is not None and (v.requires_grad or source_amplitudes.requires_grad
...
class ScalarBackwardFunc(torch.autograd.Function):
    ...
    def forward(ctx, gwfc, gwfp, gpsiy, gpsix, gzetay, gzetax, grad_r, v, ay,
        ...
        r = torch.zeros(nt // step_ratio, n_shots, *v.shape)
        ...
        for i in range(nt // step_ratio - 1, -1, -1):
            ...
            if v.is_cuda:
                ...
                chunk = grad_r[i * step_ratio:(i + 1) *
                               step_ratio].contiguous()
            else:
                #grad_v += (wfc * dwdv[:, i]).sum(dim=0) * step_ratio
                chunk = grad_r[:, i * step_ratio:(i + 1) *
                               step_ratio].contiguous()
            backward(v2dt2.data_ptr(), chunk.data_ptr(), gwfc.data_ptr(),
                     ...
                     receivers_i.data_ptr(), 1 / dy, 1 / dx, 1 / dy**2,
                     1 / dx**2, step_ratio, n_shots, ny, nx, 0,
                     n_receivers_per_shot, step_ratio, False, pml_y0, pml_y1,
                     pml_x0, pml_x1, aux)
            # We need to swap some Tensors so that we can flip-flop
            # between memory locations at each timestep
            if step_ratio % 2 != 0:
                gwfc, gwfp = gwfp, gwfc
                gpsiy, gpsiyn = gpsiyn, gpsiy
                gpsix, gpsixn = gpsixn, gpsix
                gzetax, gzetaxn = gzetaxn, gzetax
                gzetay, gzetayn = gzetayn, gzetay

        s = (slice(None), slice(fd_pad, -fd_pad), slice(fd_pad, -fd_pad))
        if nt % 2 == 0:
            return grad_v, grad_f, gwfc[s], -gwfp[s], gpsiy[s], gpsix[
                s], gzetay[s], gzetax[s], r
        else:
            return grad_v, grad_f, gwfp[s], -gwfc[s], gpsiyn[s], gpsixn[
                s], gzetayn[s], gzetaxn[s], r


def scalar_func(*args):
    return ScalarForwardFunc.apply(*args)

def get_src_rcv(v: Tensor,
    grid_spacing: Union[int, float, List[float], Tensor],
    dt: float,
    d_true_refl: Tensor,
    source_amplitudes: Optional[Tensor] = None,
    ...
    # Setup (copied from scalar function)
    (models, source_amplitudes_l, wavefields,
     pml_profiles, sources_i_l, receivers_i_l,
     dy, dx, dt, nt, n_shots,
     step_ratio, model_gradient_sampling_interval,
    ...
    # Run forward modelling, saving snapshots of 2 * v * dt^2 * laplacian(source wavefield)
    (wfc, wfp, psiy, psix, zetay, zetax, receiver_amplitudes, dwdv) = \
        scalar_func(
            v, source_amplitudes, wfc, wfp, psiy, psix, zetay, zetax, ay, ax, by, bx, dbydy, dbxdx, sources_i, receivers_i, dy, dx, dt, nt, step_ratio * model_gradient_sampling_interval, accuracy, pml_width_list, n_shots
        )
    ...
    gpsiy = torch.empty(0)
    gpsix = torch.empty(0)
    gzetay = torch.empty(0)
    gzetax = torch.empty(0)
    source_amplitudes_requires_grad = False

    # Calculate adjoint source for l = 0.5*||d_pred - d_true||_2^2
    d_true_refl = deepwave.common.upsample(d_true_refl,
                  step_ratio,
                  freq_taper_frac=freq_taper_frac,
                  time_pad_frac=time_pad_frac,
                  time_taper=time_taper)
    if d_true_refl.device == torch.device('cpu'):
        d_true_refl = torch.movedim(d_true_refl, -1, 1)
    else:
        d_true_refl = torch.movedim(d_true_refl, -1, 0)

    grad_r = d_true_refl
    rcv_out = ScalarBackwardFunc.forward(None, gwfc, gwfp, gpsiy, gpsix, gzetay, gzetax, grad_r, v, ay,
                ax, by, bx, dbydy, dbxdx, sources_i, receivers_i, dwdv,
                source_amplitudes, wfc, wfp, psiy, psix, zetay, zetax, dy, dx,
                dt, nt, n_shots, step_ratio, accuracy, pml_width_list,
                source_amplitudes_requires_grad)
    return dwdv.detach(), rcv_out[-1].detach()
